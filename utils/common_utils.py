...
def plot_image_grid(images_np, nrow=8, factor=1, interpolation="bicubic"):
...
def interpolate_lr(image, image_lr, alpha=0.0):
    """Interpolate image with image_lr, assuming that image_lr is half the resolution of image
    Returns alpha*image + (1-alpha)*image_lr."""
    return np.clip(
        alpha * image + (1 - alpha) * zoom(image_lr, [1, 2, 2], order=0),
        0, 1)
