"""Code from the original Deep Image Prior implementation."""
...
def denoise(fname, plot=False, stopping_mode="AMNS"):
    ...
    def closure():
        ...
        method_noise_mse = np.sqrt(compare_mse(img_noisy_np - out_avg.detach().cpu().type(torch.FloatTensor).numpy()[0], np.zeros(img_np.shape, dtype=np.float32))*255**2)
